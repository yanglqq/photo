...
Options {
    ...
    function run(resultType) {
        manager.execute(function(){
            ...
            switch (combox.index) {
            case 0:
                uri = filters.idealHighpassFilter(currentImage, cutoff.value, resultType, basedOnHSI);
                break;
            case 1:
                uri = filters.butterworthHighpassFilter(currentImage, cutoff.value, order.value, resultType, basedOnHSI);
                break;
            case 2:
                uri = filters.gaussianHighpassFilter(currentImage, cutoff.value, resultType, basedOnHSI);
                break;
            }
            if (generated)
