...
Options {
    ...
    function run(resultType) {
        manager.execute(function(){
            ...
            switch (combox.index) {
            case 0:
                uri = filters.bandPassFilter_Ideal(currentImage, cutoff.value, bandWidth.value, resultType, basedOnHSI);
                break;
            case 1:
                uri = filters.bandPassFilter_Butterworth(currentImage, cutoff.value, order.value, bandWidth.value, resultType, basedOnHSI);
                break;
            case 2:
                uri = filters.bandPassFilter_Gaussian(currentImage, cutoff.value, bandWidth.value, resultType, basedOnHSI);
                break;
            }
            if (generated)
