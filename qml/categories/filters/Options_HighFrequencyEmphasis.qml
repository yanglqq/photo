...
Options {
    ...
    function run(resultType) {
        manager.execute(function(){
            ...
            switch (combox.index) {
            case 0:
                uri = filters.highFrequencyEmphasis_Ideal(currentImage, cutoff.value, offset.value, contribution.value, resultType, basedOnHSI);
                break;
            case 1:
                uri = filters.highFrequencyEmphasis_Butterworth(currentImage, cutoff.value, order.value, offset.value, contribution.value, resultType, basedOnHSI);
                break;
            case 2:
                uri = filters.highFrequencyEmphasis_Gaussian(currentImage, cutoff.value, offset.value, contribution.value, resultType, basedOnHSI);
                break;
            }
            if (generated)
    ...
    optionModel: ObjectModel {
        ...
        Slider {
            id: contribution
            width: contentWidth
            label: "Contribution Of High Freqency"
            minValue: 0
            maxValue: 10
            value: 1
            bins: 20
