...
Options {
    ...
    function run(type) {
        ...
        manager.execute(function(){
            ...
            switch (combox.index) {
            case 0:
                uri = filters.notchPassFilter_Block(imageItem.owner, image, type, basedOnHSI);
                break;
            case 1:
                uri = filters.notchPassFilter_Ideal(imageItem.owner, image, cutoff.value, type, basedOnHSI);
                break;
            case 2:
                uri = filters.notchPassFilter_Butterworth(imageItem.owner, image, cutoff.value, order.value, type, basedOnHSI);
                break;
            case 3:
                uri = filters.notchPassFilter_Gaussian(imageItem.owner, image, cutoff.value, type, basedOnHSI);
                break;
            }
