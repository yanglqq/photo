    ...
    onApply: {
        if (checker.checked) {
            ...
            manager.execute(function() {
                var uri = multiresolution.inverseFastWaveletTransform(selected[0], selected[1], selected[2], selected[3], combox.index, borderType);
                manager.addImage(uri, null, selected[3]);
                return true;
            });
        } else {
            manager.execute(function(){
                var uris = multiresolution.fastWaveletTransform(manager.currentImage, combox.index, borderType);
